	...
	const ulint*	offsets)
{
	...
	for (ulint i = 0; i < prebuilt->n_template; i++) {
		...
		ulint sec_index_field_no = templ->rec_prefix_field_no;

		/** Condition (1) : is the field in the index. */
		if (sec_index_field_no == ULINT_UNDEFINED) {
			return false;
		}
		...
		ulint rec_size = rec_offs_nth_size(offsets,
						      sec_index_field_no);
		const dict_field_t* field = dict_index_get_nth_field(
						index, sec_index_field_no);
		...
		if (rec_size < max_chars) {
			...
			continue;
		} else if (rec_size * templ->mbminlen
			   >= field->prefix_len) {
